...
func MutilTableExecutor(report *core.Report) {
	...
	rows, cols := 100, 5
	table := Cgopdf.NewTable(cols, rows, 415, lineHeight, report)
	...
	for i := 0; i < rows; i += 5 {
		...
		case 0:
			for row := 0; row < 5; row++ {
				for col := 0; col < cols; col++ {
					...
					cell := table.NewCell()
					txt := Cgopdf.NewTextCell(table.GetColWidth(i+row, col), lineHeight, lineSpace, report).SetBackColor(GetRandColor())
		...
		case 1:
			...
			t00 := Cgopdf.NewTextCell(table.GetColWidth(i+0, 0), lineHeight, lineSpace, report).SetBackColor(GetRandColor())
			t01 := Cgopdf.NewTextCell(table.GetColWidth(i+0, 1), lineHeight, lineSpace, report).SetBackColor(GetRandColor())
			t03 := Cgopdf.NewTextCell(table.GetColWidth(i+0, 3), lineHeight, lineSpace, report).SetBackColor(GetRandColor())
			t21 := Cgopdf.NewTextCell(table.GetColWidth(i+2, 1), lineHeight, lineSpace, report).SetBackColor(GetRandColor())
			t31 := Cgopdf.NewTextCell(table.GetColWidth(i+3, 1), lineHeight, lineSpace, report).SetBackColor(GetRandColor())
			t41 := Cgopdf.NewTextCell(table.GetColWidth(i+4, 1), lineHeight, lineSpace, report).SetBackColor(GetRandColor())
		...
		case 2:
			...
			t00 := Cgopdf.NewTextCell(table.GetColWidth(i+0, 0), lineHeight, lineSpace, report).SetBackColor(GetRandColor())
			t03 := Cgopdf.NewTextCell(table.GetColWidth(i+0, 3), lineHeight, lineSpace, report).SetBackColor(GetRandColor())
			t20 := Cgopdf.NewTextCell(table.GetColWidth(i+2, 0), lineHeight, lineSpace, report).SetBackColor(GetRandColor())
			t21 := Cgopdf.NewTextCell(table.GetColWidth(i+2, 1), lineHeight, lineSpace, report).SetBackColor(GetRandColor())
			t33 := Cgopdf.NewTextCell(table.GetColWidth(i+3, 3), lineHeight, lineSpace, report).SetBackColor(GetRandColor())
			t40 := Cgopdf.NewTextCell(table.GetColWidth(i+4, 0), lineHeight, lineSpace, report).SetBackColor(GetRandColor())
