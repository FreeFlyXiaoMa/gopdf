...
func SimpleTableExecutor(report *core.Report) {
	...
	//table := gopdf.NewTable(5, 6, 415, lineHeight, report)
	...
	table := Cgopdf.NewTable(2, 1, 415, lineHeight, report)
	table.SetMargin(core.Scope{})
	w1 := table.GetColWidth(0, 1)
	e1 := Cgopdf.NewTextCell(w1, lineHeight, lineSpace, report)
	cel1 := table.NewCell()
	cel1.SetElement(e1)

	w2 := table.GetColWidth(0, 2)
	e2 := Cgopdf.NewTextCell(w2, lineHeight, lineSpace, report)
	cel2 := table.NewCell()
	cel2.SetElement(e2)
